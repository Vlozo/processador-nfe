...
def open_client_management():
    ...
    entry_client = ttk.Entry(field_frame, textvariable=entry_var, width=30)
    ...
    ttk.Button(field_frame, text="Registrar", command=register_client).pack(side="left")
    ...
    def update_client_list():
        ...
        for i, ct in enumerate(clients):
            ...
            row.pack(fill="x", pady=4, padx=(2, 5))
...
icon_path = os.path.join(base_path, "static", "icon-nfe.ico")
...
# Campo Diretório
ttk.Label(container, text="Diretório:").pack(anchor="w", padx=10, pady=(10, 0))
dir_var = StringVar(value="")
dir_path = ttk.Entry(container, textvariable=dir_var, width=65)
dir_path.pack(anchor="w", padx=10)
ttk.Button(container, text="Abrir diretório", command=open_directory).pack(anchor="w", padx=10, pady=20)

# Opção de buscar por subdiretórios
ttk.Label(container, text="Buscar por arquivos aninhados em subdiretórios?").pack(anchor="w", padx=10, pady=(10, 0))
entry_recursive = BooleanVar(value="True")
frame_recursive = Frame(container)
frame_recursive.pack(anchor="w", padx=10)
ttk.Radiobutton(frame_recursive, text="Sim", variable=entry_recursive, value=True).pack(side="left")
ttk.Radiobutton(frame_recursive, text="Não", variable=entry_recursive, value=False).pack(side="left")

# Lista de Clientes
ttk.Button(container, text="Gerenciar Identificadores", command=open_client_management).pack(anchor="w", padx=10, pady=(20, 0))
